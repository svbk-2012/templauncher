...
def install_dependencies():
    ...
    required_packages = ['tkinterdnd2', 'psutil']
...
class LauncherWrapper:
    def __init__(self, root):
        self.root = root
        self.root.title("Universal App Runner - Launcher & App Mode")
        self.root.geometry("800x600")
        self.root.configure(bg='#2b2b2b')
        
        # Create directories
        self.downloads_dir = Path.home() / "Downloads" / "UniversalRunner"
        self.downloads_dir.mkdir(exist_ok=True)
        self.temp_dir = Path.home() / "Downloads" / "UniversalRunner" / "temp"
        self.temp_dir.mkdir(exist_ok=True)
        
        # Mode and process info
        self.mode = "launcher"  # "launcher" or "app"
    ...
    def setup_ui(self):
        ...
        title_label = tk.Label(self.root, text="Universal App Runner", 
        ...
        tk.Label(mode_frame, text="Select Mode:", 
                font=("Arial", 12), bg='#2b2b2b', fg='white').pack(side='left', padx=10)
        
        self.mode_var = tk.StringVar(value="launcher")
        
        launcher_radio = tk.Radiobutton(mode_frame, text="Launcher Mode (Download & Save)", 
                                      variable=self.mode_var, value="launcher",
                                      command=self.change_mode,
                                      bg='#2b2b2b', fg='white', selectcolor='#2b2b2b')
        launcher_radio.pack(side='left', padx=5)
        
        app_radio = tk.Radiobutton(mode_frame, text="App Mode (Run Directly)", 
                                  variable=self.mode_var, value="app",
                                  command=self.change_mode,
                                  bg='#2b2b2b', fg='white', selectcolor='#2b2b2b')
        app_radio.pack(side='left', padx=5)
    ...
    def change_mode(self):
        """Handle mode change between launcher and app runner"""
        self.mode = self.mode_var.get()
        self.update_drop_text()
        self.update_button_text()
        
    def update_drop_text(self):
        """Update drop area text based on current mode"""
        if self.mode == "launcher":
            text = "Drag and drop launcher files here\n(.exe, .msi, .zip, etc.)\n\nDownload & Save Mode"
        else:
            text = "Drag and drop applications/games here\n(.exe, .bat, etc.)\n\nRun Directly Mode"
        self.drop_label.configure(text=text)
        
    def update_button_text(self):
        """Update button text based on current mode"""
        if self.mode == "launcher":
            self.launch_btn.configure(text="Launch Download")
        else:
            self.launch_btn.configure(text="Run Application")
